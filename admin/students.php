...
if(isset($_SESSION['name']) == 'admin') {
    ...
    if(isset($_POST['deleted'])) {
        ...
        $sql = "SELECT EXISTS (SELECT * FROM `sfmu.students` WHERE id = '$did') as `row_exists`  LIMIT 1";
        $result = $connect->query($sql);

        if($result->fetch_assoc()['row_exists'] > 0) {
            $sql = "DELETE FROM `sfmu.students` WHERE id = $did";
            $result = $connect->query($sql);
            if($result) {
                $notification = 'Student Deleted Successfully!';
                $border = 'success';
            } else {
                $notification = 'Error Deleting Student!';
                $border = 'danger';
            }
        } else {
            $notification = "Student doesn't Exists!";
            $border = 'danger';
        }
    }
    ...
    if(isset($_POST['addd'])) {
        ...
        $sql = "SELECT EXISTS (SELECT * FROM `sfmu.students` WHERE name = '$admin') as `row_exists`  LIMIT 1";
        $result = $connect->query($sql);

        if($result->fetch_assoc()['row_exists'] > 0) {
            $notification = 'Student Already Exists!';
            $border = 'danger';
         }
         else { 
            $sql = "INSERT INTO `sfmu.students`(name,department,password) VALUES ('$admin','$department','$password')";
            $result = $connect->query($sql);
            if($result) {
                $notification = 'Student Added Successfully!';
                $border = 'success';
            } else {
                $notification = 'Error Adding Student!';
                $border = 'danger';
            }
        }
            ...
            <nav class="navbar-dark">
                <ul class="navbar-nav">
                    ...
                    <li>
                        ...
                        <li>
                            <a class="nav-link px-3 active" href="students.php">
                                <span class="me-2"><i class="bi bi-arrow-bar-right"></i></span>
                                <span>Students</span>
                            </a>
                        </li>
    ...
    <main class="mt-5 pt-3">
        <div class="container-fluid">
            <!-- Toast Notification -->
                ...
                <?php } ?>
            <div class="row">
                <div class="col-md-12 fw-bold fs-3">Students List</div>
                <section class="mt-2 px-2">
                    <div class="d-flex justify-content-between mb-4">
                      ...
                      <div class="col-sm-4 me-1">
                            <input type="text" placeholder="Enter Student Name or ID" class="form-control" id="search">
                        </div>
                    </div>
                    </div>
                </section>
