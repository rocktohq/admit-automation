...
if(isset($_SESSION['role']) == 1) {
    ...
    if(isset($_POST['deleted'])) {
        ...
        $sql = "SELECT EXISTS (SELECT * FROM `sfmu.admin` WHERE id = '$did') as `row_exists`  LIMIT 1";
        $result = $connect->query($sql);

        if($result->fetch_assoc()['row_exists'] > 0) {
            $sql = "DELETE FROM `sfmu.admin` WHERE id = $did";
            $result = $connect->query($sql);
            if($result) {
                $notification = 'Admin Deleted Successfully!';
                $border = 'success';
            } else {
                $notification = 'Error Deleting Admin!';
                $border = 'danger';
            }
        } else {
            $notification = "Admin doesn't Exists!";
            $border = 'danger';
        }
    }
    ...
    if(isset($_POST['addd'])) {
        ...
        $sql = "SELECT EXISTS (SELECT * FROM `sfmu.admin` WHERE name = '$admin') as `row_exists`  LIMIT 1";
        $result = $connect->query($sql);

        if($result->fetch_assoc()['row_exists'] > 0) {
            $notification = 'Admin Already Exists!';
            $border = 'danger';
         }
         else { 
            $sql = "INSERT INTO `sfmu.admin`(name,department,password) VALUES ('$admin','$department','$password')";
            $result = $connect->query($sql);
            if($result) {
                $notification = 'Admin Added Successfully!';
                $border = 'success';
            } else {
                $notification = 'Error Adding Admin!';
                $border = 'danger';
            }
        }
        ...
        <div class="offcanvas-body p-0">
            <nav class="navbar-dark">
                <ul class="navbar-nav">
                    ...
                    <li>
                        <li>
                            <a class="nav-link px-3 active" href="admins.php">
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalAddLabel">Add New Admin</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="col-md-10 mb-2">
                            ...
                            <select class="form-select" name="department" id="department" required>
                                ...
                                <?php 
                                    $query = "SELECT `name`, `short_name` FROM `sfmu.departments`";
    ...
    <main class="mt-5 pt-3">
        <div class="container-fluid">
            <!-- Toast Notification -->
                ...
                <?php } ?>
            <div class="row">
                <div class="col-md-12 fw-bold fs-3">Administrators</div>
                ...
                <?php 
                   // Now Fetch Data form Admin
                    $sql = "SELECT * FROM `sfmu.admin`";
                    $result = $connect->query($sql);
                    if(!$result->num_rows > 0) {
                            echo '<div class="mt-5 text-center h1"> No Admin found!</div>';
                        } else {    ?>
